  ...
  def get_value(value, options)
    if value.present? && options.present?
      if value.is_a?(Array)
        ...
        v = options.select{|option| option['id'] == value.to_s.strip || option['display_text'].present? && option['display_text'].strip == value.to_s.strip || option['display_text'].present? && option['display_text'].strip == value.to_s.capitalize.strip}.first.try(:[], 'id')
        if v == 'true'
          v = true
        elsif v == 'false'
          v = false
